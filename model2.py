...
class DCGenerator2(tfkl.Layer):
    def __init__(self):
        ...
        self.netG = tf.keras.Sequential([
            tfkl.Conv2DTranspose(self.ngf * 8, kernel_size=4, stride=1, padding='valid', bias=False),
            tfkl.BatchNormalization(),
            tfkl.ReLU(),
            # state size. (ngf*8) x 4 x 4
            tfkl.Conv2DTranspose(self.ngf * 4, kernel_size=4, stride=2, padding='same', bias=False),
            tfkl.BatchNormalization(),
            tfkl.ReLU(),
            # state size. (ngf*4) x 8 x 8
            tfkl.Conv2DTranspose(self.ngf * 2, kernel_size=4, stride=2, padding='same', bias=False),
            tfkl.BatchNormalization(),
            tfkl.ReLU(),
            # state size. (ngf*2) x 16 x 16
            tfkl.Conv2DTranspose(self.ngf, kernel_size=4, stride=2, padding='same', bias=False),
            tfkl.BatchNormalization(),
            tfkl.ReLU(),
            # state size. (ngf) x 32 x 32
            tfkl.Conv2DTranspose(self.num_channels, kernel_size=4, stride=2, padding='same', bias=False),
...
class Concat_embed(tfkl.Layer):
    ...
    def call(self, inp, embed):
        ...
        hidden_concat = tf.concat([inp, replicated_embed], 1)
...
class DCDiscriminator2(tfkl.Layer):
    def __init__(self):
        ...
        self.netD_1 = tf.keras.Sequential([
            ...
            tfkl.Conv2D(self.ndf, 4, 2, padding='same', bias=False),
            tfkl.LeakyReLU(0.2),
            # state size. (ndf) x 32 x 32
            tfkl.Conv2D(self.ndf * 2, 4, 2, padding='same', bias=False),
            tfkl.BatchNormalization(),
            tfkl.LeakyReLU(0.2),
            # state size. (ndf*2) x 16 x 16
            tfkl.Conv2D(self.ndf * 4, 4, 2, padding='same', bias=False),
            tfkl.BatchNormalization(),
            tfkl.LeakyReLU(0.2),
            # state size. (ndf*4) x 8 x 8
            tfkl.Conv2D(self.ndf * 8, 4, 2, padding='same', bias=False),
            tfkl.BatchNormalization(),
            tfkl.LeakyReLU(0.2),
        ])
        ...
        self.netD_2 = tf.keras.Sequential(
            ...
            tfkl.Conv2D(1, 4, 1, padding='valid', bias=False),
...
class GAN2(tf.keras.Model):
    ...
    def call(self, x, embed, wrong_images, train_step_new=True):
        if train_step_new:
